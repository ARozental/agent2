...
class AgentLevel(nn.Module):
    def __init__(self, level, num_letters):
        ...
        self.classifier1w = nn.Parameter((2.2/5.0) * torch.ones(1, requires_grad=True))  # with sane init
        self.classifier1b = nn.Parameter((-1.1/5.0) * torch.ones(1, requires_grad=True))  # with sane init

        if Config.join_texts:
            self.join_classifier_w = nn.Parameter((2.2/5.0) * torch.ones(1, requires_grad=True))  # with sane init
            self.join_classifier_b = nn.Parameter((-1.1/5.0) * torch.ones(1, requires_grad=True))  # with sane init
    ...
    def eos_classifier1(self, dot):
        ...
        return F.elu(dot * self.classifier1w * torch.sign(self.classifier1w) * 5.0) + self.classifier1b * 5.0

    def join_classifier(self, dot):
        # needed to make sure w1 can never be negative
        return F.elu(dot * self.join_classifier_w * torch.sign(self.join_classifier_w)* 5.0 ) + self.join_classifier_b * 5.0
