...
class Config:
    ...
    node_sizes = [4096, 768, 4096, 1000, 1000]  # How many nodes to process at a time at each level => todo: change, each here limits the other
    ...
    mini_batch_size = 4096 #max number of max_agent_level document, not working as intender but has an effect: final number is ~1.7 times higers, can be higher than batch size like when we get wiki articles as input but only doing up to level 1 (sentneces). should be at least as high as corresponding node size
