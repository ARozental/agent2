...
def calc_coherence_loss(agent_level,matrices,mask, eos_positions,embeddings):
    ...
    changed_examples = torch.rand(batch,1).to(Config.device).round() #50% of examples don't change at all, move to config?
    change_probs = torch.rand(batch,1).to(Config.device)
