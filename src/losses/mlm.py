...
def calc_mlm_loss(agent_level,matrices,mask,embeddings,labels):
    #matrices => [batch,seq_length,vec_size], embeddings => [seq_length,vec_size]
    ...
    random_indexes = torch.fmod(torch.randperm(batch* seq_length), embeddings.shape[0]) #can in theory be the pad token here
    ...
    #print(embeddings.shape,transformed.shape,logits.shape,mlm_losses)
