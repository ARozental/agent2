...
def calc_reconstruction_loss(agent_level, matrices, decompressed, mask, eos_positions, join_positions, embeddings,
    ...
    if Config.use_pndb1 is not None and agent_level.level == 1:
        post_decoder = pndb.get_data_from_A_matrix(pndb.create_A_matrix(matrices, mask), post_decoder)
    ...
    if agent_level.level > 0:
        rc_loss, total_rcd_loss = calc_lower_rc_loss(agent_level, reencoded_matrices, mask, agent_level.previous_level,
                                                     post_decoder)
    else:
        rc_loss, total_rcd_loss = calc_rc_loss(agent_level, reencoded_matrices, mask, agent_level.previous_level,
                                               post_decoder)

    if Config.join_texts and agent_level.level > 0:
        rj_loss = calc_join_loss(agent_level, post_decoder, join_positions)
    else:
        rj_loss = torch.tensor([0.0] * post_decoder.size(0)).to(Config.device)
